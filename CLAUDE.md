...
/projects              Our Projects — Zakat / Sadaqah Jariyah / Rehabilitation showcase, plus a Gallery (uploaded media + YouTube playlists) (public)
...
**Our Projects page's Gallery** (`app/(public)/projects/page.tsx` → `lib/gallery.ts`, `app/(public)/projects/components/GallerySection.tsx`): sits below the Zakat/Sadaqah Jariyah/Rehabilitation showcase on `/projects` and has two tabs, switched client-side in `GallerySection` (`activeTab` state, no route change):
  - **Uploaded Media tab** (`UploadedMediaTab.tsx`): reads the public, admin-populated `gallery` Firestore collection. `page.tsx` fetches the first page server-side via `getGalleryItems()` (`lib/gallery.ts`, `GALLERY_PAGE_SIZE` = 12, ordered by `createdAt` desc) and passes it to `GallerySection` as `initialItems`/`initialCursor`; further pages are fetched client-side straight from Firestore's client SDK (no API route needed — unlike Blogger/YouTube, Firestore's client SDK has no CORS restriction and holds no secret) using `afterCreatedAt` as a serializable cursor.
  - **YouTube Playlists tab** (`YouTubePlaylistsTab.tsx`): fetches from `/api/youtube/playlists`, which proxies `lib/youtube.ts`'s call to the YouTube Data API v3 (`GET .../playlists?channelId=…`, mirroring `lib/blogger.ts`'s shape: native `fetch`, `revalidate: 3600`, safe empty-result fallback on error) — this route exists to keep `YOUTUBE_API_KEY` off the browser, not for CORS. `siteConfig.youtube.channelId` supplies the channel; the key must be restricted to "YouTube Data API v3" in Google Cloud Console.

  Admin write path for the gallery: `app/dashboard/components/GalleryScreen.tsx` (see Admin dashboard below) is the only place gallery items are created or deleted. The public Gallery tab is read-only.
