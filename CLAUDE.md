...
- `lib/siteConfig.ts` — language-independent facts (phone/bKash/Nagad numbers, email, social handles, `blogUrl`, `youtube.channelId`) plus `projectMedia`, an array of `{ image, video }` (paths under `public/projects/`) read by index into `dictionaries.ts`'s `projects.categories` to fill each project's media panel on the Our Projects page — `null`/`null` renders a themed icon placeholder until real files are added. It also has `projectBanners`, a separate per-index array of wide hero-banner paths (under `public/projects/banners/`) for the detail pages. It's deliberately separate from `projectMedia` so a category can have a card photo without a banner, or the other way round, and `null` falls back to the plain green pattern. The hero is much wider than most photos, so banners are `object-cover`-cropped. An entry can be `{ src, position }` (a CSS `object-position`, e.g. `"center top"`) instead of a plain path, to keep an off-center part of the image, like baked-in title text, in view. The brochure only gives social **handles**, not URLs. `youtube.href` has since been filled in, but `facebook.href` is still `""`, and **`youtube.channelId` is still `""`**. `lib/youtube.ts` returns an empty result when either the channel ID or `YOUTUBE_API_KEY` is missing, so the Gallery's playlists tab stays empty until the real channel ID is set.
- `app/(public)/components/` — `Navbar` (nav links + `LanguageToggle`, its own file here), `Footer`, and `HomeContent` (the group-root page's content, colocated here since it lives directly in `app/(public)/`). `Navbar`/`Footer` are shared by every page in the group via `app/(public)/layout.tsx`.
...
**Our Projects page's Gallery** (`lib/gallery.ts`, `app/(public)/projects/components/GallerySection.tsx`): sits below the project grid on `/projects` (not per-category — one sitewide gallery) and has two tabs, switched client-side in `GallerySection` (`activeTab` state, no route change):
  ...
  The sitewide gallery shows every item except those flagged `hideFromMainGallery: true`, whether or not they have project tags. Hidden items still appear on their tagged project pages. The flag is only ever stored as `true`: absent means shown, and un-hiding `deleteField()`s it. It's filtered in JS inside `getGalleryItems()`, not in the query. Firestore's `!=` never matches docs that lack the field, so `where("hideFromMainGallery", "!=", true)` would drop almost every item. `getGalleryItems()` therefore reads raw pages (up to 5 per call) until it has a full page of visible items, and returns the last *raw* doc read as the cursor, so a page can come back short.

  Admin write path for the gallery: `app/dashboard/components/GalleryScreen.tsx` (see Admin dashboard below) is the only place gallery items are created, deleted, or assigned to a project. Uploads are tagged via a checklist in `AddGalleryItemModal`. Existing items are re-tagged via the grid's tag button, which calls `updateDoc`. It writes the `projectSlugs` array (or `deleteField()` when no project is ticked) and `hideFromMainGallery` (from the "General (main gallery slider)" checkbox), and always `deleteField()`s the legacy `projectSlug`, so re-tagging converts old items to the new shape. Both use the shared `GalleryProjectChecklist` component, which reads its choices from `galleryProjectOptions.ts`, which is built from the English dictionary's `projects.categories`. The public Gallery tab is read-only. The admin grid itself (`GalleryScreen.tsx`) uses small thumbnails (3–10 responsive columns) rather than a large grid, anticipating many uploads over time.
...
**Gallery write flow** (`GalleryScreen.tsx` + `AddGalleryItemModal.tsx`): uploads go straight from the browser to Cloudinary, never through this app's server, to avoid Vercel serverless body/duration limits on large videos.
  ...
  3. Deletion (`GalleryScreen`'s trash button → `DeleteConfirmationModal`) calls `POST /api/gallery/delete` (also bearer-token-authenticated) to remove the Cloudinary asset by `publicId`, then deletes the Firestore doc client-side — the API route only ever touches Cloudinary, never Firestore. Bulk delete ("Select" mode in the grid) reuses that same single-item path, one item at a time in sequence; items that fail stay selected for retry, and changing the project filter clears the selection.
...
**`app/dashboard/components/`** — besides `FoundationDashboard`, notable pieces: `Header`/`Footer` are the *admin* chrome (distinct from `app/(public)/components/Navbar`/`Footer`), and `galleryProjectOptions.ts` (the Gallery's project-tag choices) lives here too. `TransactionSection` owns search/sort/pagination (10 items/page) over the transactions object it's handed for the current project+year+type; `DeleteConfirmationModal` is reused for per-transaction, per-project, *and* per-gallery-item deletes. `YearlySummaryScreen` is a separate full-screen view (toggled via `showYearlySummary` state inside `FoundationDashboard`, not a route) that aggregates totals across all years/projects.
