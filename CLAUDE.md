...
`lib/utils/projectStats.ts` holds pure, state-free helpers (`calculateTotals`, `calculateProjectTotals`, `getAvailableYears`, `getProjectYears`) that derive income/expense/balance totals from the `{ income, expenses }` shape — used by `FoundationDashboard`/`ProjectControls` instead of inlining the aggregation logic.
