...
**Our Projects page** (`app/(public)/projects/page.tsx` → `ProjectsContent.tsx`): renders a responsive grid of category cards, one per entry in `dictionaries.ts`'s `projects.categories` (see Types below) — each card links to that category's own detail route, `/projects/[slug]`. `CATEGORY_ICONS` (a fixed lucide-react icon per array index) and `siteConfig.ts`'s `projectMedia` (per-index `{ image, video }`, currently all `null` placeholders) must stay in the same order as `projects.categories` across all three files.

**Project detail pages** (`app/(public)/projects/[slug]/page.tsx` → `components/ProjectDetailContent.tsx`): a static route per category (`generateStaticParams()` reads `siteConfig.ts`'s `PROJECT_CATEGORY_SLUGS`, a language-independent slug list kept in sync with `projects.categories[].slug`); an unknown slug 404s via `notFound()`. Each page renders a banner, description, an objectives/beneficiaries/budget/area/duration info-card sidebar, a donate CTA, a gallery placeholder, an impact-stats placeholder, and an FAQ accordion — all sourced from that category's `ProjectCategory` dictionary entry (`dictionaries.ts`) via `projectDetail` for the static section labels. The gallery and impact-stats sections intentionally render "coming soon" placeholders rather than fabricated numbers, since there's no per-category media or real statistics yet — swap them for real content (and wire in a real gallery, e.g. by tagging `gallery` Firestore docs with a category) once the foundation supplies it.
